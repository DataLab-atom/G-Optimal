...
def static_weight_loss(model, optimizer, loader, device,args):
    ...
    for it, data in enumerate(loader):
        ...
        G_att_loss =  gaussian_kl(edge_att,data.split_e)
        s_loss = F.kl_div(xs_logs, un_one_hot_target, reduction='batchmean')#-(un_one_hot_target * xs_logs).sum(dim=-1).mean()
...
def mgda_loss(model, optimizer, loader, device,args):
    ...
    for it, data in enumerate(loader):
        ...
        G_att_loss =  gaussian_kl(edge_att,data.split_e)
        s_loss = F.kl_div(xs_logs, un_one_hot_target, reduction='batchmean')
        m_loss = F.nll_loss(xm_logs, one_hot_target) 
        ...
        gn = MinNormSolver.gradient_normalizers(grads, loss_data, 'none')
        
        for t in loss_data:
            for gr_i in range(len(grads[t])):
                grads[t][gr_i] = grads[t][gr_i] / gn[t].to(grads[t][gr_i].device)
        sol, _ = MinNormSolver.find_min_norm_element([grads[t] for t in loss_name])
        ...
        for key in loss_name:
            loss.append(saved_loss[key]*sol[key]*3)
            mgda[key].append(sol[key])
        #loss.append(G_att_loss)
        # loss = saved_loss['s']*sol['s'] + saved_loss['m']*sol['m']
        loss = sum(loss)
