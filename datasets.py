...
def get_mnistsp(args):
    ...
    train_val = MNIST75sp(args.data_root + 'MNISTSP/', mode='train')
    ...
    test_dataset = MNIST75sp(args.data_root + 'MNISTSP/', mode='test')
    color_noises = torch.load('MNISTSP/raw/mnist_75sp_color_noise.pt').view(-1,3)
...
def get_SPMotif(args):
    ...
    traing_dataset = SPMotif(args.data_root+ f'SPMotif-{args.bias}/', mode='train')
    val_dataset = SPMotif(args.data_root+ f'SPMotif-{args.bias}/', mode='val')
    test_dataset = SPMotif(args.data_root + f'SPMotif-{args.bias}/', mode='test')
...
def get_sst(args):
    ...
    dataset = get_dataset(dataset_dir=args.data_root + '', dataset_name='Graph_SST2', task=None)
