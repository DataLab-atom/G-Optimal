...
from torch_geometric.nn import global_max_pool,global_mean_pool, global_add_pool, GINConv, GATConv
...
class Split_sub_encoder(torch.nn.Module):
    def __init__(self, args,gfn=False,edge_norm=True):
        ...
        self.args = args
        hidden = args.hidden
        self.global_pool = pool_map[args.global_pool]# global_add_pool
...
class Template(torch.nn.Module):
    ...
    def forward(self, data,dict_out = False):
        ...
        out['xs'] = xs
        xm = xm + xs*torch.rand_like(xs) #self.MS_connet(xs , xm)
        out['xms'] = xm
...
class GCNNet(torch.nn.Module):
    ...
    def __init__(self,args, num_features,
        ...
        self.global_pool = pool_map[args.global_pool]
